  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-slate-500 text-sm mt-1">Order {receipt.orderNumber}</p>
      </div>

      {/* Receipt content */}
      <div className="border border-slate-200 rounded-xl p-6 space-y-4 print-receipt">
        <div className="text-center border-b border-dashed border-slate-300 pb-4">
          <p className="font-bold text-lg font-mono">Brand OS</p>
          <p className="text-xs text-slate-500">{formatDateTime(receipt.createdAt)}</p>
          {receipt.eventName && <p className="text-xs text-blue-600 font-medium mt-1">Event: {receipt.eventName}</p>}
        </div>
            ...
            <div key={idx} className="flex justify-between text-sm">
              <div>
                ...
                {(item.size || item.color) && (
                  <p className="text-xs text-slate-500">{[item.size, item.color].filter(Boolean).join(' · ')} × {item.quantity}</p>
                )}
              </div>
        ...
        <div className="border-t border-dashed border-slate-300 pt-3 space-y-1.5 text-sm">
          ...
          <div className="flex justify-between font-bold text-base border-t border-slate-200 pt-1.5 mt-1.5">
            <span>TOTAL</span>
        ...
        <div className="text-center text-xs text-slate-400 border-t border-dashed border-slate-300 pt-3">
