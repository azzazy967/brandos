  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-slate-200">
      ...
      <div className="p-4 border-b border-slate-200 shrink-0">
        ...
        {eventName && (
          <p className="text-xs text-blue-600 mt-1 font-medium">Event: {eventName}</p>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {items.length === 0 ? (
          ...
          items.map(item => (
            <div key={item.productId} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50">
              {item.imageUrl ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                {(item.size || item.color) && (
                  <p className="text-xs text-slate-500">{[item.size, item.color].filter(Boolean).join(' · ')}</p>
                )}
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="h-8 w-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:border-[#2563EB] hover:text-[#2563EB] transition-all cursor-pointer"
                >
                  <Minus size={14} />
                </button>
                ...
                <button
                  ...
                  className="h-8 w-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:border-[#2563EB] hover:text-[#2563EB] transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Plus size={14} />
                </button>
                <button
                  onClick={() => removeItem(item.productId)}
                  className="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all cursor-pointer ml-1"
                >
                  <X size={14} />
                </button>
              </div>
            ...
            <div className="flex gap-2">
              <div className="flex rounded-lg border border-slate-200 overflow-hidden flex-1">
                <button
                  ...
                  className={cn('flex-1 text-xs font-medium py-2 transition-colors cursor-pointer', discountTypeLocal === 'fixed' ? 'bg-[#2563EB] text-white' : 'text-slate-600 hover:bg-slate-50')}
                >
                  EGP
                </button>
                <button
                  onClick={() => handleDiscountChange(discountInput, 'percent')}
                  className={cn('flex-1 text-xs font-medium py-2 transition-colors cursor-pointer', discountTypeLocal === 'percent' ? 'bg-[#2563EB] text-white' : 'text-slate-600 hover:bg-slate-50')}
                >
                  %
                </button>
              </div>
              <input
                ...
                placeholder="0"
                className="flex-1 h-10 rounded-lg border border-slate-200 px-3 text-sm focus:outline-none focus:border-[#2563EB] transition-colors"
                min="0"
