...
const SIZE_COLORS_LIGHT = ['#2563EB', '#3B82F6', '#60A5FA', '#93C5FD', '#BFDBFE', '#DBEAFE']
const SIZE_COLORS_DARK = ['#3B82F6', '#60A5FA', '#93C5FD', '#6DB3F8', '#4A9CF5', '#2E86E8']

export default function SizeIntelligence() {
  const [collections, setCollections] = useState<CollectionSizes[]>([])
  const [loading, setLoading] = useState(true)
  const { resolved } = useThemeStore()
  const SIZE_COLORS = resolved === 'dark' ? SIZE_COLORS_DARK : SIZE_COLORS_LIGHT
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Size Intelligence</h1>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Size Intelligence</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Understand which sizes sell best per collection to optimize future production runs</p>
      </div>
              ...
              <Card key={col.collection}>
                <CardHeader>
                  ...
                  <div className="flex gap-3 text-xs mt-1">
                    {topSize && <span className="text-green-600 dark:text-green-400">Top: <strong>{topSize.size}</strong> ({(topSize.sellThroughPct ?? 0).toFixed(0)}%)</span>}
                    {worstSize && <span className="text-amber-600 dark:text-amber-400">Slowest: <strong>{worstSize.size}</strong> ({(worstSize.sellThroughPct ?? 0).toFixed(0)}%)</span>}
                  </div>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {col.sizes.map(s => (
                      <div key={s.size} className="text-center p-2 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                        <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{s.size}</p>
                        <p className="text-xs text-green-600 dark:text-green-400">{s.unitsSold} sold</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{s.currentStock} left</p>
                      </div>
                    ))}
                  </div>
                </CardContent>
